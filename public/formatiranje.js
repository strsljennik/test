...
// Definicija funkcije
function formatGuestNickname(guestNumber, format) {
    const guestElement = document.getElementById(`guest-${guestNumber}`);
    if (guestElement) {
        guestElement.style.fontWeight = format.bold ? 'bold' : 'normal';
        guestElement.style.fontStyle = format.italic ? 'italic' : 'normal';
        guestElement.style.color = format.color ? format.color : 'black';
    }
}

// Poziv funkcije za formatiranje
// Primer: Formatiraj gosta sa brojem 1
formatGuestNickname(1, { bold: true, italic: true, color: 'red' });
